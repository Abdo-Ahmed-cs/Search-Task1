...
def depthFirstSearch(problem: SearchProblem):
    ...
    if problem.isGoalState(startingpoint):
        return []
    ...
    while not stack.isEmpty():
        currtState, actions = stack.pop()

        if currtState not in visited:
            visited.append(currtState)
            if problem.isGoalState(currtState):
                return actions
            for nextState, action, cost in problem.getSuccessors(currtState):
